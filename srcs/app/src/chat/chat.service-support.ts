...
export class ChatServiceSupport {
  ...
  static {
    ...
    ChatServiceSupport.TYPES.set(ChatAction.CHAT_INFO, [ChatType.PUBLIC, ChatType.PROTECTED, ChatType.PRIVATE, ChatType.DIRECT]);
  ...
  async isBlockedUser(user: User, targetUser: User) {
    let res = await this.findDirectChatLink(targetUser, user);
    if (!res || !res.userStatus)
      return false;
    return res.userStatus == UserChatStatus.MUTED;
  }
}
